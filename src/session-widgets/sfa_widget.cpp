...
void SFAWidget::setAuthType(const int type)
{
    ...
    int authType = type;
    // TODO 这里不应该加上无密码登陆和自动登陆的判断，应该让插件自行处理，这里的逻辑需要重构一下
    ...
    if (m_customAuth && !m_customAuth->showSwitchButton()) {
...
void SFAWidget::updateFocus()
{
    for (int i = 0; i < m_mainLayout->layout()->count(); ++i) {
        ...
        if (authModule && authModule->authType() == m_chooseAuthButtonBox->checkedId()) {
            if (authModule->authType() == AT_Custom && m_customAuth && !m_customAuth->showSwitchButton()) {
                return;
            }
            setFocus();
...
void SFAWidget::onRequestChangeAuth(const int authType)
{
    qInfo() << Q_FUNC_INFO
            << "authType" << authType
            << "m_chooseAuthButtonBox->isEnabled()" << m_chooseAuthButtonBox->isEnabled()
